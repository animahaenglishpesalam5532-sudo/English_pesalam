...
import { Plus, Search, Presentation, Download, Eye, Edit2, Trash2, ExternalLink, Image as ImageIcon, Upload, Loader2, X, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function PPTManager() {
  ...
  const [selectedPPT, setSelectedPPT] = useState<PPTData | null>(null)
  ...
  const fetchPPTs = async () => {
    try {
      ...
      toast.error('Failed to fetch PPTs: ' + error.message)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && selectedPPT) {
        const { error } = await supabase
          .from('ppts')
          .update(payload)
          .eq('id', selectedPPT.id)
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this PPT?')) return
  ...
  const openEditModal = (ppt: PPTData) => {
    setSelectedPPT(ppt)
    setFormData({
      name: ppt.name,
      description: ppt.description,
      image_url: ppt.image_url,
      marked_price: ppt.marked_price,
      selling_price: ppt.selling_price,
      is_visible: ppt.is_visible,
      is_featured: ppt.is_featured
    })
    setIsEditing(true)
    setIsModalOpen(true)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">PPT Manager</h1>
          <p className="text-gray-500">Manage PowerPoint presentations and digital guides.</p>
        </div>
        <button
                  ...
                  <tr key={ppt.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        {ppt.image_url ? (
                          <img src={ppt.image_url} alt={ppt.name} className="w-12 h-12 object-cover rounded-lg border border-gray-100" />
                        ) : (
                        ...
                        <div className="max-w-xs">
                          <div className="font-semibold text-gray-900 truncate">{ppt.name}</div>
                          <div className="text-xs text-gray-500 truncate">{ppt.description}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-blue-600">{ppt.selling_price}</span>
                        {ppt.marked_price && (
                          <span className="text-xs text-gray-400 line-through">{ppt.marked_price}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        <span className={`w-fit px-2.5 py-0.5 rounded-full text-[10px] font-medium ${ppt.is_visible ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                          {ppt.is_visible ? 'Visible' : 'Hidden'}
                        </span>
                        {ppt.is_featured && (
      ...
      <Modal
        ...
        title={isEditing ? 'Edit Product' : 'Add New Product (PPT/PDF)'}
      >
