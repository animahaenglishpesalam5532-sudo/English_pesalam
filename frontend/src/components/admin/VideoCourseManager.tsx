...
import { Plus, Search, Video, Download, Eye, Edit2, Trash2, ExternalLink, Image as ImageIcon, Upload, Loader2, X, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function VideoCourseManager() {
  const [courses, setCourses] = useState<VideoCourseData[]>([])
  ...
  const [selectedCourse, setSelectedCourse] = useState<VideoCourseData | null>(null)
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await uploadImage(fd, 'video-courses')
  ...
  const openEditModal = (course: VideoCourseData) => {
    setSelectedCourse(course)
    setFormData({
      name: course.name,
      description: course.description,
      image_url: course.image_url,
      marked_price: course.marked_price,
    ...
    <div className="space-y-6">
                  ...
                  <tr key={course.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        {course.image_url ? (
                          <img src={course.image_url} alt={course.name} className="w-12 h-12 object-cover rounded-lg border border-gray-100" />
                        ) : (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-blue-600">{course.selling_price}</span>
                        {course.marked_price && (
                          <span className="text-xs text-gray-400 line-through">{course.marked_price}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        <span className={`w-fit px-2.5 py-0.5 rounded-full text-[10px] font-medium ${course.is_visible ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
      ...
      >
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">Marked Price (e.g. ₹2999)</label>
              <input
                type="text"
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">Selling Price (e.g. ₹999)</label>
              <input
                type="text"
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 space-y-3">
                <label className="flex flex-col items-center justify-center gap-2 px-4 py-4 rounded-xl border-2 border-dashed border-blue-200 bg-blue-50/50 cursor-pointer hover:border-blue-500 transition-colors">
                  ...
                  <span className="text-xs text-gray-500">{isUploading ? 'Uploading...' : 'Upload Thumbnail'}</span>
