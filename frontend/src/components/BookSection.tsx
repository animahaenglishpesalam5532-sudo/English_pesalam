  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-start w-full order-2 md:order-1 relative z-10">
        {/* Title */}
        <h2 className="mb-4 md:mb-6 pb-2 leading-tight tracking-tight text-3xl font-extrabold">
          <span
...
export function BookSection({ books = [] }: BookSectionProps) {
  const displayBooks = books.length > 0 ? books.filter(b => b.is_visible).slice(0, 2) : [fallbackBook];

  if (displayBooks.length === 0) return null;

  return (
    <section id="books" className="py-8 px-4 w-full max-w-md md:max-w-6xl mx-auto">
